...
(function () {
	...
	var helper = {
		...
		eventListener: (function () {
			...
			return {
				add: function(relatedJgalleryInstance, relatedItem, element, type, handler, options, removeWhenOccurred) {

					eventListeners.push([relatedJgalleryInstance, relatedItem, element, type, handler, options]);
					
					element.addEventListener(type, removeWhenOccurred ? function(event) {
						helper.eventListener.remove(relatedJgalleryInstance, relatedItem, element, type, handler, options);
						return handler(event);
					} : handler, options);
				},
				remove: function(relatedJgalleryInstance, relatedItem, element, type, handler, options) {
					var pos = eventListeners.length;
					var eventListener;
					while (pos--) {
						eventListener = eventListeners[pos];
						if (relatedJgalleryInstance === undefined || relatedJgalleryInstance === eventListener[0]) {
		...
		expectNaturalImageSize: (function () {
			...
			return function (jgalleryInstance, item, img, onReadyCallback) {
				var obj = {
					img: img,
				...
				var onLoad = function () {
					obj.ready = true;
					helper.eventListener.remove(jgalleryInstance, item, obj.imgB, 'error', onError, false);
				};
				var onError = function () {
					obj.ready = true;
					obj.error = true;
					helper.eventListener.remove(jgalleryInstance, item, obj.imgB, 'load', onLoad, false);
				};

				helper.eventListener.add(jgalleryInstance, item, obj.imgB, 'load', onLoad, false, true);
				helper.eventListener.add(jgalleryInstance, item, obj.imgB, 'error', onError, false, true);
	...
	window.Jgallery = function (element, options) {
		...
		if (isNewInstance) {
			...
			// set jgallery id and version as element attributes (so it's easy recognizable outside)
			self.element.setAttribute('data-jgalleryid', self.id);
			self.element.setAttribute('data-jgalleryversion', self.version);
	...
	Jgallery.prototype = {
		...
		destroy: function () {
			...
			// remove attributes
			this.element.removeAttribute('data-jgalleryid');
			this.element.removeAttribute('data-jgalleryversion');
		...
		_syncElements: function (childElement) {
			...
			for (pos = 0; pos < children.length; pos++) {
				...
				if (children[pos].jgalleryItem !== undefined) {
		...
		_resetItemsDOM: function (items) {
			...
			while (pos--) {
				...
				delete item.contentElement.jgalleryItem;

				// remove existing jgallery classes from element
				helper.changeMultipleClassNames(item.contentElement, false, this._itemVisibilityClassesArray.concat(['jgalleryItem', 'jgalleryItemLoading', 'jgalleryItemInitializing', 'jgalleryItemReady', 'jgalleryItemComplete']));
				...
				item.contentElement.removeAttribute('data-jgalleryitemid');
			}
		},
		...
		_getOptionParametersFromElement: function (element, layout, attributeName) {
			...
			if (attributeName) {
				...
				while (pos--) {
					if (attributes[pos].name.substr(0, 12) === 'data-jgallery-') {
				...
				while (pos--) {
					if (!helper.inArray(filterControls, this._filterControls[pos])) {
						switch (this._filterControls[pos].nodeName + this._filterControls[pos].type) {
							...
							case 'INPUTcheckbox':
							case 'INPUTradio':
								helper.eventListener.remove(this, null, this._filterControls[pos], 'change', this._filterControls[pos].jgalleryFilterControlHandler);
								break;
							case 'INPUTbutton':
							case 'INPUTsubmit':
							case 'BUTTONbutton':
							case 'BUTTONsubmit':
							case 'BUTTONreset':
								helper.eventListener.remove(this, null, this._filterControls[pos], 'click', this._filterControls[pos].jgalleryFilterControlHandler);
								break;
							default:
								helper.eventListener.remove(this, null, this._filterControls[pos], 'input', this._filterControls[pos].jgalleryFilterControlHandler);
						}
				...
				while (pos--) {
					if (!helper.inArray(this._filterControls, filterControls[pos])) {
						...
						switch (filterControls[pos].nodeName + filterControls[pos].type) {
							...
							case 'INPUTcheckbox':
							case 'INPUTradio':
								helper.eventListener.add(this, null, filterControls[pos], 'change', filterControls[pos].jgalleryFilterControlHandler);
								break;
							case 'INPUTbutton':
							case 'INPUTsubmit':
							case 'BUTTONbutton':
							case 'BUTTONsubmit':
							case 'BUTTONreset':
								helper.eventListener.add(this, null, filterControls[pos], 'click', filterControls[pos].jgalleryFilterControlHandler);
								break;
							default:
								helper.eventListener.add(this, null, filterControls[pos], 'input', filterControls[pos].jgalleryFilterControlHandler);
						}
...
// var x = document.querySelector("[content-id='tab-2']");	 
// x.id  = 'galleryTab';

// var x = document.getElementById("galleryTab");
...
new JgalleryLayout({
	...
	handler: function(renderObject) {
		...
		var // get layout options
			autoFontSize = !!renderObject.jgalleryInstance.getOption('autoFontSize'),
			desiredHeight = parseInt(renderObject.jgalleryInstance.getOption('desiredHeight')),
			hideOnMissingImage = !!renderObject.jgalleryInstance.getOption('hideOnMissingImage'),
			spaceBetween = parseInt(renderObject.jgalleryInstance.getOption('spaceBetween')),
