...
if ( $action == NULL ) {
	if (file_exists($lmm_pro_readme) || (file_exists($lmm_pro_readme_four)) ) {
		...
		echo '<p>' . __('The free version of this plugin is retired, meaning it will no longer be updated for new features (<a href="https://www.mapsmarker.com/v3.12.3" target="_blank">details</a>). If you’d like a faster plugin with dedicated support and new features added frequently, try Maps Marker Pro: Start a free 30-day-trial without any obligation.','lmm') . '</p>';
		if ( current_user_can( 'install_plugins' ) ) {
	...
	if ($action == 'upgrade_to_pro_version') {
		...
		if (file_exists($lmm_pro_readme)) {
			...
			echo '<p>' . sprintf(__('The pro plugin package could not be downloaded automatically. Please download the plugin from <a href="%1s">%2s</a> and upload it to the directory /wp-content/plugins on your server manually','lmm'), $dl, $dl) . '</p>';
		}
	}
}
?>
