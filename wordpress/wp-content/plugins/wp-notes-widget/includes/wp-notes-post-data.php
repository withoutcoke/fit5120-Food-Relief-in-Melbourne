<?php
  ...
  if (!function_exists('getNotePostData')) {
    ...
    function getNotePostData($id) {
      ...
      $wp_notes_data['media_embed']           = !empty($wp_notes_data['media_embed_type'])    ? sanitize_text_field($wp_notes_data['media_embed_type']) : '';
      $wp_notes_data['media_video_link']      = !empty($wp_notes_data['media_video_link'])    ? sanitize_text_field($wp_notes_data['media_video_link']) : '';
      $wp_notes_data['media_audio_id']        = !empty($wp_notes_data['media_audio_id'])      ? sanitize_text_field($wp_notes_data['media_audio_id']) : '';
      $wp_notes_data['media_audio_src']       = '';
      $wp_notes_data['media_video_embed_src'] = '';
      ...
      if ((bool)$wp_notes_data['media_audio_id']) {
        $wp_notes_data['media_audio_src'] = wp_get_attachment_url($wp_notes_data['media_audio_id'] );
         
      }
      

      // Set up value for $wp_notes_data['media_embed'], check to see which is active, if any. Need to check for previous values, before new logic
      if ( (empty($wp_notes_data['media_embed']) || $wp_notes_data['media_embed'] == 'image' ) && !empty($wp_notes_data['image_id']) )   {
      
        $wp_notes_data['media_embed'] = 'image';
      
      } elseif ($wp_notes_data['media_embed'] == 'video' && !empty($wp_notes_data['media_video_link']) ) {
      
        $wp_notes_data['media_embed'] = 'video';
        $wp_notes_data['media_video_embed_src'] = WP_Notes_Public::get_video_embed_link($wp_notes_data['media_video_link']);
      
      } elseif ($wp_notes_data['media_embed'] == 'audio' && !empty($wp_notes_data['media_audio_id'])) {
      
        $wp_notes_data['media_embed'] = 'audio';
  ...
  if (!function_exists('getNotePostTwitterData')) {
    ...
    function getNotePostTwitterData($id) {
      ...
      if (!$wp_notes_twitter_data) {
        $wp_notes_twitter_data = array();
      }
